...
class Ieee1212ConfigRom():
    ...
    def parse(self, rom):
        ...
        info['bus-info'] = self._parse_bus_info(rom[4:4 + bus_info_length])
        rom = rom[4 + bus_info_length:]
        info['root-directory'] = self._parse_root_directory(rom)
    ...
    def _parse_csr_offset(self, key, value, rom):
        raise OSError('Leaf of CSR is not supported.')
    ...
    def _parse_directory_entries(self, directory_length, rom):
        ...
        #
        KEY_DEFS = {
            0x01:   ('descriptor',                  0b1100),
            0x02:   ('bus-dependent-info',          0b1101),
            0x03:   ('vendor',                      0b1101),
            0x04:   ('hardware-version',            0b0001),
            # 0x05-06: reserved.
            0x07:   ('module',                      0b1100),
            # 0x08-0b: reserved.
            0x0c:   ('node-capabilities',           0b0001),
            0x0d:   ('eui-64',                      0b0100),
            # 0x0e-10: reserved.
            0x11:   ('unit',                        0b1000),
            0x12:   ('specifier-id',                0b0001),
            0x13:   ('version',                     0b0001),
            0x14:   ('dependent-info',              0b1111),
            0x15:   ('unit-location',               0b0100),
            0x17:   ('model',                       0b0001),
            0x18:   ('instance',                    0b1000),
            0x19:   ('keyword',                     0b0100),
            0x1a:   ('feature',                     0b1000),
            0x1b:   ('extended-rom',                0b0100),
            0x1c:   ('extended-key-specifier-id',   0b0001),
            0x1d:   ('extended-key',                0b0001),
            0x1e:   ('extended-data',               0b1111),
            0x1f:   ('modifiable-descriptor',       0b0100),
            0x20:   ('directory-id',                0b0001),
            # 0x21-2f: reserved.
        ...
        while length < directory_length:
            ...
            if definition and (1 << type) & definition[1]:
                if definition[0] != 'descriptor':
                    key_name = definition[0]
            else:
                key_name = '0x{0:x}'.format(key)

            entries.append([key_name, type_name, type_parser(key, value, rom)])
